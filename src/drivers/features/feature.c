...
{
        SerialPrint("INFO: Finding Features\r\n");
        for (size_t i = 0; i < 64; ++i)
        {
                size_t index = i & 31;
                Features[i] = (i > 31) ? FeatureIsPresentRCX(index) : FeatureIsPresentRDX(index);
                if (Features[i])
                {
                        const char *const name = FeatureName(i);
                        SerialPrint("INFO: Feature ");
                        SerialPrint(name);
                        SerialPrint("\r\n");
                }
        }
        ...
        {
                SerialPrint("INFO: SSE Is available, attempting to enable\r\n");
                SSEEnable();
                asm volatile(" fxsave %0 " ::"m"(fxsave_region));
                SerialPrint("INFO: SSE Is enabled\r\n");
        }
        else
        {
                SerialPrint("PANIC: SSE Is required for this operating system\r\n");
                hcf();
        }

        if (Features[2])
        {
                SerialPrint("INFO: DE Is available\r\n");
                FeatureCR4Enable(3);
        }

        if (Features[5])
        {
                SerialPrint("INFO: MSR Is available\r\n");
        }
        else
        {
                SerialPrint("PANIC: MSR Is not available\r\n");
                hcf();
        }
}
