...
void GDIPixel32(SURFACE *Surface, float X, float Y, float Z, ColourRGBA Col)
{
...
void GDIPixel(SURFACE *Surface, float X, float Y, float Z, ColourRGBA Col)
{
        switch (Surface->BPP)
        {
        case 32:
                GDIPixel32(Surface, X, Y, Z, Col);
                break;
        case 24:
                SerialPrint("'24 bits'\n");
                goto err;
...
void GDIDrawRect(SURFACE *Surface, RECT *Rect)
{
        if (!Surface || !Rect)
                return;
        ...
        for (float y = minY; y <= maxY; y += 1.0)
        {
                ...
                for (float x = minX; x <= maxX; x += 1.0)
                {
                        ...
                        GDIPixel(Surface, x, y, z, final);
                }
        }
}

void GDIBlitSurface(DISPLAY *Display, SURFACE *Surface)
{
        if (!Display || !Surface)
                return;
